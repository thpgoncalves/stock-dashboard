...
from data_acquisition.get_stock_data import get_comparison_data, get_stock_data
from data_processing.charts import create_chart, plot_comparison_chart
...
def main():
    ...
    with tab1:  # "Solo" stock view tab
        st.markdown(
            "<h1 style='text-align: center;'>Select Stocks Tickers and Parameters</h1>",
            unsafe_allow_html=True,
        )
        ...
        for col, stock_number in zip([col1, col2], range(1, 3)):  # loop for stock 1, 2
            with col:
                ...
                company_name = st.text_input(
                    f"Search Stock {stock_number} by Name:",
                    key=f"company_search_{stock_number}",
                ...
                if company_name:
                    ...
                    if stock_options:
                        selected_stock = st.selectbox(
                            f"Select Stock {stock_number}:",
                            stock_options,
                            key=f"ticker_selected_{stock_number}",
                        )
                    else:
                ...
                if selected_period == "Custom":
                    period_input = st.text_input(
                        "Enter custom period:",
                        placeholder="1d, 1wk, 1mo, 1y, ytd...",
                        key=f"stock_period_{stock_number}",
                    )
                    interval_input = st.text_input(
                        "Enter custom interval:",
                        placeholder="1m, 1h, 1d, 1wk, 1mo...",
                        key=f"stock_interval_{stock_number}",
                    )
                ...
                if ticker:
                    ...
                    if not data.empty:
                        ...
                        st.plotly_chart(
                            chart, use_container_width=True, key=f"chart{stock_number}"
                        )
    ...
    with tab2:  # Comparison tab
        ...
        comparison_company_name = st.text_input(
        ...
        if comparison_company_name:
            ...
            if comparison_stock_options:
                selected_comparison_stock = st.selectbox(
                    "Select Stock:",
                    options=comparison_stock_options,
                    key="comparison_ticker_selected",
                )
            else:
                st.warning("No stocks found. Try another search.")
        ...
        manual_tickers = st.text_area(
            ...
            "BERK34.SA, IVVB11.SA, RAIZ4.SA, OPCT3.SA",
        )  # Change default stocks
        manual_ticker_list = [
            ticker.strip() for ticker in manual_tickers.split(",") if ticker.split()
        ]

        if (
            selected_comparison_stock
            and selected_comparison_stock not in manual_ticker_list
        ):
        ...
        comparison_period, comparison_interval = PERIOD_OPTIONS[
            selected_comparison_period
        ]

        if selected_comparison_period == "Custom":
            comparison_period_input = st.text_input(
                "Enter custom period:",
                placeholder="1d, 1wk, 1mo, 1y, ytd...",
                key="custom_comparison_period",
            )
            comparison_interval_input = st.text_input(
                "Enter custom interval:",
                placeholder="1m, 1h, 1d, 1wk, 1mo...",
                key="custom_comparison_interval",
            )

            comparison_period = (
                comparison_period_input if comparison_period_input else "1y"
            )
            comparison_interval = (
                comparison_interval_input if comparison_interval_input else "1d"
            )
            ...
            if manual_ticker_list:
                ...
                with contextlib.redirect_stderr(stderr_buffer):
                    df_comparison = get_comparison_data(
                        manual_ticker_list, comparison_period, comparison_interval
                    )
